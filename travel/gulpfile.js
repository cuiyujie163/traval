  ...
  connect.server({//启动本地server
    ...
    middleware:function(){
      return[function(req,res,next){//接受响应、请求体、next回调函数
        ...
        path=path=="/" ? "/index.html" : path;//入口文件名
        //获取运行时参数
...
/*//启动项目
gulp.task("serve",["rev","connect","watch"]); //执行一个server任务 包括build与connect服务器任务 watch监听
...
gulp.task("build",function(){
  gulp.src(["src/js/a.js","src/js/b.js"])
    .pipe(uglify())  //压缩
    .pide(concat("a+b.js"))  //合并
    .pide(gulp.dest("src/js/build/"))  //输出
})

//3、angularjs项目的压缩
gulp.task("miniJs",["miniCss"],function(){
  gulp.src([""])//操作 src方法里可操作数组传多个文件
    .pipe(ngAnnotate())//编译控制器
    .pipe(ngmin())//压缩下
    .pipe(uglify())//压缩
    .pipe(concat("a+b.min.js"))//合并
    .pipe(rev()) //加密
